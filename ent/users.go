	...
	for i := range columns {
		...
		case users.FieldVerificationTokenExpiry:
			if value, ok := values[i].(*sql.NullTime); !ok {
				...
				u.VerificationTokenExpiry = new(time.Time)
				*u.VerificationTokenExpiry = value.Time
			}
		...
		case users.FieldPasswordResetTokenExpiry:
			if value, ok := values[i].(*sql.NullTime); !ok {
				return fmt.Errorf("unexpected type %T for field password_reset_token_expiry", values[i])
			} else if value.Valid {
				u.PasswordResetTokenExpiry = new(time.Time)
				*u.PasswordResetTokenExpiry = value.Time
			}
		case users.FieldMetadata:
...
func (u *Users) String() string {
	...
	builder.WriteString(", ")
	if v := u.VerificationTokenExpiry; v != nil {
	...
	builder.WriteString(", ")
	if v := u.PasswordResetTokenExpiry; v != nil {
		builder.WriteString("password_reset_token_expiry=")
		builder.WriteString(v.Format(time.ANSIC))
	}
	builder.WriteString(", ")
